...
6. Before **every new agent/worker**—implementation, research, review, evaluation, or independent/nested work—apply `model-router` to the candidates actually discovered and usable in the current harness. Do not encode a provider/model preference order in bootstrap policy. Publish a user-visible `model.selected` event with the actual selected model and thinking level when known (otherwise `unknown`). A persistent continuation of an already-bound thread keeps its routing unless replaced, forked, or explicitly reconsidered.
...
- Apply the `model-router` skill before every new worker. Use only dynamically discovered usable candidates and the router's generic policy; do not add a provider/model preference order here. Once a persistent worker thread is bound to a profile, keep it sticky unless rerouting is justified.
