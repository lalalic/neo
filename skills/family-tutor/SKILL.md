...
Create a family tutoring system in which each child has an independent ChatGPT Project, a dedicated warm ChatGPT tab, rotating tutor threads, and one local durable `memory.md`. Parents can observe useful learning signals and set goals through Discord, and a long-running bridge connects Discord to ChatGPT without requiring OpenAI API billing.
...
7. rotate overly long/noisy ChatGPT threads while preserving continuity through local `memory.md`;
...
- Local durable learner context lives only at `<instance-dir>/<child-id>/memory.md`. Do not create local transcript/session folders or a conversation `state.json`. ChatGPT owns thread history.
- When the dedicated tab is at the Project home/new-thread state, the bridge sends the current `memory.md` and the first student turn together. Once ChatGPT owns an active thread, later turns send only the new student content and attachment grounding; the bridge does not prepend memory again. The tutor may replace durable memory by emitting `<FAMILY_TUTOR_MEMORY>...complete Markdown...</FAMILY_TUTOR_MEMORY>`; the bridge strips the block from the child response and atomically writes the replacement.
