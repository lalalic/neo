...
RUN_ID = os.environ.get("DRAMA_RUN_ID", "current")
...
def concept(force=False):
    ...
    write_sidecar(output, "taotao-concept", "image", prompt, ["runs/<run-id>/characters/taotao/character.yaml"], log)
...
def tts(episode, line_id, force=False):
    ...
    write_sidecar(output, f"{episode}-{line_id}-audio", "audio", selected["text"], [f"runs/<run-id>/episodes/{episode}/script.json"], log)
...
def shot(episode, shot_id, force=False):
    ...
    source_assets = [f"runs/<run-id>/episodes/{episode}/shots.json"]
    ...
    if continuity_from:
        ...
        source_assets.append("runs/<run-id>/generated/images/taotao-concept.png")
    else:
        command += ["--mode", "ti2vid"]
    output.parent.mkdir(parents=True, exist_ok=True)
...
def audit_run(run_id):
    ...
    result = {"run_id": run_id, "episode": episode, "core_state": run.get("state"), "delivery": delivery, "valid": not errors, "errors": errors, "warnings": warnings}
...
def main():
    ...
    try:
        if args.command == "status":
            ...
            validate()
            record("prepare", "succeeded", f"runs/{RUN_ID}/story/series.json", "run source contracts validated")
            print("prepare: ok")
