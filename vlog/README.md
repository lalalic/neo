...
The tracked project contains only the reusable production system. Every actual vlog execution—including source media, episode Markdown, SQLite state, generated narration/BGM, previews, reviews, publish receipts, and final video—belongs under ignored `runs/<run-id>/`.
...
- `series/` — reusable series continuity/configuration.
...
python3 -m neo_vlog --db runs/demo/state/vlog.sqlite --config config/routing.json init
python3 -m neo_vlog --db runs/demo/state/vlog.sqlite --config config/routing.json event \
  --key demo-arrival-001 --series neo-build-log \
  --payload '{"media_path":"runs/demo/assets","source":"bootstrap-demo"}'
python3 -m neo_vlog --db runs/demo/state/vlog.sqlite --config config/routing.json run
python3 -m neo_vlog --db runs/demo/state/vlog.sqlite status
```

If `--db` is omitted, the CLI defaults to `runs/current/state/vlog.sqlite`.
